...
int cast(object me, object target)
{
	...
	mapping exit;
	int mana_cost = 25 + me->query("mana_factor");
	int force_cost =  25 + me->query("force_factor");
	...
	if( !target
		...
		||   target->is_corpse()
		||   target==me)
		return notify_fail("你要去撞谁？\n");

	if(me->query_skill("pingtian-dafa", 1) < 30 )
		return notify_fail("你的平天大法火候不够，哪里施展的出！\n");

	if(me->query("mana") < mana_cost)
