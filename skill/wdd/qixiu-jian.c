...
mixed hit_ob(object me, object victim, int damage_bonus)
{
	...
	int attack = me->query_skill("yaofa") * 3;	//妖法可以通过带妖气的装备提升

	if(!userp(me) || me->query_skill_mapped("spells") != "yaofa") return 0;
	if(random(food) > 30 || !cd_start(me, "wdd_blood", CD)) return 0;

	me->add_temp("apply/sword", attack);			//增强武器
	me->add_temp("apply/yinfeng-zhua", attack);	//增强爪子

	call_out("remove_effect", CD, me, attack);

	return HIR"$N看着$n留出的血，眼中闪动着贪婪的光芒！\n"NOR;
}
...
void remove_effect(object me, int attack)
{
	if(me) {
		...
		message_vision(RED"$N眼中的光芒黯淡了下来。\n"NOR, me);
	}
}
