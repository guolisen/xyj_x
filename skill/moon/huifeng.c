...
int perform(object me, object target)
{
	...
	msg = HIW"\n$N眼见不敌，银牙一咬，突然高高纵起，凝聚全身功力，人剑合一，"
		+"化做一道白光，疾风般扑向$n！\n" NOR;
	ap = me->query_skill("snowsword", 1) / 10;
	ap = ap * ap * ap /10;
	ap = ap + (int)me->query("combat_exp") / 1000;
	dp = target->query_skill("parry",1) / 10;
	dp = dp * dp * dp /10;   
	dp = dp + target->query("combat_exp") / 1000;
	...
	me->start_busy(2);
	//me->start_busy(2+random(3));		//减少随机性和双方busy，将更实用
