...
int perform(object me, object target)
{
	...
	if( me->query_skill("fengbo-cha", 1) < 50 ) 
	...
	mypot = me->query_skill("fork");
	tapot = target->query_skill("parry");

	mypot += pow(me->query("combat_exp") * 10, 0.333);
	tapot += pow(target->query("combat_exp") * 10, 0.333);


	time = me->query_skill("fengbo-cha", 1) /10;
	if ( time > 10 ) time = 10;
	if ( time < 5  ) time = 5;
