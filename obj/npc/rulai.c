...
int reward_player ()
{
  ...
  if ((MIN_PLAYER-j) > 0)
  {
    message_vision ("$N对$n摇摇头：此处仅有数位仙贤在邀，可否再邀"+chinese_number(MIN_PLAYER-j)+
                    "位仙贤前来观礼？\n",me,who);
    if (! DEBUG)                 
      return 1;
  }
...
//add by huadao.2010.08.19
// this player must be present for rulai fo to do invite.
  invitee=me->query("last_player");
  if(!invitee) return;
...
//add by huadao.2010.08.19
// this player must be present for rulai fo to do invite.
  if(environment(invitee)!=environment(me))
      return;
