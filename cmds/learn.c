...
int main(object me, string arg)
{
	...
	if(!arg) return notify_fail("指令格式：learn <技能> from <某人> for <次数>|max\n");
	...
	if(time<1) time = 1;
	//if(time>50) time=50;	//2010.11

	if(!arg || sscanf(arg, "%s from %s", skill, teacher)!=2 )
		return notify_fail("指令格式：learn <技能> from <某人> for <次数>|max\n");
...
{
        write(@HELP
指令格式 : learn|xue <技能> from <某人> for <次数>|max
