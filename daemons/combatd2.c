...
varargs int do_attack(object me, object victim, object weapon, int attack_type)
{
	...
	if( wizardp(me) && (string)me->query("env/combat")=="verbose" ) {
		if( damage > 0 )
			tell_object(me, sprintf( GRN "me AP：%d，DP：%d，PP：%d，Mod：%d，伤害力：%d\n" NOR,
			ap, dp, pp, mod_val, damage));
		else
			tell_object(me, sprintf( GRN "me AP：%d，DP：%d，PP：%d，Mod：%d\n" NOR,
			ap, dp, pp, mod_val));
	}
	if( wizardp(victim) && (string)victim->query("env/combat")=="verbose" ) {
		if( damage > 0 )
			tell_object(victim, sprintf( GRN "target AP：%d，DP：%d，PP：%d，Mod：%d，伤害力：%d\n" NOR,
			ap, dp, pp, mod_val, damage));
		else
			tell_object(victim, sprintf( GRN "target AP：%d，DP：%d，PP：%d，Mod：%d\n" NOR,
			ap, dp, pp, mod_val));
	}
		...
		if(userp(killer) && init_killer!=-1) { // killer is user.
			...
			*/
			...
			{			
				...
				{
					str=victim->name(1)+"在"+
						MISC_D->find_place(environment(killer))+"被"+killer->name(1)+"用一招"+HIW"“"+rkmsg+"”"+HIM"送去了阎罗地府。"+str;
					CHANNEL_D->do_channel(this_object(), "rumor",str);
				}else if(weapon && !rkmsg){
					str=victim->name(1)+"在"+
						MISC_D->find_place(environment(killer))+"被"+killer->name(1)+"用"HIY+weapon->name()+HIM"送去了阎罗地府。"+str;
					CHANNEL_D->do_channel(this_object(), "rumor",str);
				}else {
					str=victim->name(1)+"在"+
