...
//冰刀构造函数
varargs void ctor(string name, int min_lvl, string file, string poison);

void create()
{
	ctor(HIW"冰刀"NOR, 100, __FILE__);
}
...
void melt()
{
	...
	if( env->is_character() )
	{//a player or a NPC.
		if( (int)env->query_skill("ningxie-force", 1) < query("ice_min_lvl")/5 )
		{
			message_vision("$N觉得身上凉凉的，湿湿的，原来是冰刀化了。\n", env);
			if(query("ice_poison"))
				env->apply_condition(query("ice_poison"), 10);
			//note, 中毒了。
...
mixed hit_ob(object me, object victim, int damage_bonus)
{
	...
	if( damage_bonus < 0 || !query("ice_poison")) return 0;
	...
	if(diff > 0 && (int)victim->query_skill("ningxie-force", 1) < query("ice_min_lvl"))
	{
		victim->apply_condition(query("ice_poison"), diff + (int)victim->query_condition(query("ice_poison")) );
		message_vision("$N身子一颤，中了刀上的寒毒！\n", victim);
	}
	else if((int)me->query_skill("ningxie-force", 1) < query("ice_min_lvl"))
	{//for very weak ppl or npc, also possible to "force back", this
		//is to ensure it'll not happen.
		me->apply_condition(query("ice_poison"), -diff + me->query_condition(query("ice_poison")) );
...
int do_duandao(string arg)
{
	...
	msg = HIW"$N运力一震，只见冰花四溅！一把冰刀顿时化为无数碎片向$n激射而去！\n"NOR;
	//produce kill here...
	target->kill_ob(me);
	...
	if(success == 1 )
	{
		...
		if(query("ice_poison"))
			target->apply_condition(query("ice_poison"),  5 + (int)me->query("force_factor")/5 + (int)target->query_condition(query("ice_poison")) );
...
varargs void ctor(string name, int min_lvl, string file, string poison)
{
	set("ice_min_lvl", min_lvl);
	set("ice_poison", poison);
	if(poison) set("no_give", 1);

	set_name(name, ({"ice blade", "blade", "dao"}));
	set_weight(1000);
