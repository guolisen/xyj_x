	...
	if(!raw) {
		...
		if( mapp(skills) ) {
			...
			if( mapp(skill_map) ) {
				...
				s += query_temp("apply/" + skill_map[skill]);	//2010.11 for apply/spi_skill
			}
		}
		return s;
	}
...
varargs int query_skill2(string skill, int raw)					//2012.05 firefox
{
	if(!raw) {
		int s = query_temp("apply/" + skill);		
		s += skills[skill];		
		s += skills[skill_map[skill]] / 4;
		s += query_temp("apply/" + skill_map[skill]);
		return s;
	}
	return skills[skill];
}
