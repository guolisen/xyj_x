...
{
	string ob = query("vendor_goods/" + what);
	if(stringp(ob))
...
varargs int complete_trade(object me, string what, int n)
{
	string ob_file;
	int price;
	...
	if( stringp(ob_file = query("vendor_goods/" + what)) ) {
		...
		object ob = new(ob_file);

		if(n > 1) {
			ob->set_amount(n);
			if(ob->query_amount() != n) return notify_fail("这东西只单卖。\n");
		}
		if(n < 1) n = 1;

		if(!ob->move(me)) return 0;
		price = ob->query("value") * n;

		if(sale_price > 5) sale_price = 5;
		...
		message_vision("$N向$n买下" + chinese_number(n) + or2(ob->query("base_unit"), ob->query("unit"))
			+ ob->query("name") + "。\n", me, this_object() );
...
{
	object who = this_player();
	int ratio, max_price = 20;

	if(getuid(who) != owner_id) return notify_fail("只有店主能用这个命令。\n");

	if(!arg || sscanf(arg,"%d", ratio) != 1 || ratio < 1 || ratio > max_price)
		return notify_fail("用法: set_price <1-20> \n");
	...
	if(ratio<10)
		...
		string str = chinese_number(ratio/10);
...
void reload(string filename)
{
	...
	if(owner_id) set_new_owner(owner_id, owner_name, owner_long);
...
{ 
	...
	if(price_ratio>22) price_ratio=price_ratio*9/10; // decrease price after death.
	save();
