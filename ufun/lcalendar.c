...
参考文献：
	C++实现农历算法：http://unix-cd.com/unixcd12/article_6027.html
*/

/********************************接口定义***********************************/

//公历转农历
varargs mapping luanr_date(mixed date_time);
...
//计算这个公历日期是一年中的第几天
private int day_of_solar_year(int year, int month, int day )
{
	//为了提高效率，记录每月一日是一年中的第几天
	int* normal_yday = ({1, 32, 60, 91, 121, 152, 182, 213, 244, 274, 305, 335});
	//闰年的情况
	int* leap_yday = ({1, 32, 61, 92, 122, 153, 183, 214, 245, 275, 306, 336});
	int* arr = normal_yday;
...
mapping luanr_date_(int solar_year, int solar_month, int solar_day)
{
	mapping luanr_date = ([ LT_YEAR : solar_year ]);
	int luanr_month = 1;
	int leap_month;
	...
	if(today_luanr_yd < 0) {
		...
		year_total_day = day_of_solar_year(luanr_date[LT_YEAR], 12, 31);         
	...
	luanr_date[LT_MDAY] = today_luanr_yd;
	//处理闰月
	leap_month = (lunar_years[year_index] >> 20) & 0xf;
	if(leap_month > 0 && leap_month < luanr_month )	{    
		luanr_month--;
		//如果当前月为闰月，设置闰月标志
		if(luanr_month == leap_month) luanr_date[LT_LEAP] = 1;         
	}
	//assert(leap_month <= 12);
	luanr_date[LT_MON] = luanr_month - 1; //月份0表示1月
	return luanr_date;
}
...
varargs mapping luanr_date(mixed date_time)
{
	mixed lt;
	if(intp(date_time)) {
		date_time = or2(date_time, time());
		lt = localtime(date_time);
	}
	else if(arrayp(date_time)) lt = date_time;
	else if(mapp(date_time)) lt = date_time;
	else return 0;

	return luanr_date_(lt[LT_YEAR], lt[LT_MON] + 1, lt[LT_MDAY]);
}

void show(mapping lt)
{
...
{
	show(luanr_date());
	show(luanr_date(time()));
	show(luanr_date(localtime(time())));
